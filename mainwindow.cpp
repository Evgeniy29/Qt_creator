...
{
    entries.append({"", "Task", "Date", "Progress"});
    Load();
}
...
QStringList MainWindow::parseEntry(const QString& line) const
{
    ...
    if(pos1 > 0 && pos2 > pos1)
    {
        ...
        QString progress = line.right(line.size() - 1 - pos2 );
        entry.append(task);
        entry.append(date);
...
void MainWindow::onAdd(const QString& task, const QString& date, const QString& progress)
{
    beginInsertRows(QModelIndex(), rowCount(), rowCount());
    entries.append(QStringList{"", task, date, progress});
    endInsertRows();
}
